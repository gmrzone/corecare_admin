    ...
    <>
      <form className={`ui big form ${style['login-form']}`} onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className={`field ${errors.password && 'error'}`}>
          ...
          <input type="text" placeholder="Password" {...register('password', {required: {value: true, message: "Cannot login without a password"}, minLength: {value: 5, message: "Password cannot be less then 5 char."}})} />
        </div>
        <p>Forgot Password</p>
        {formError || errors?.number || errors?.password ? (
          <div className="ui red message">
            {formError?.message || errors?.number?.message || errors?.password?.message}
          </div>
        ) : (
          ''
        )}
      ...
      <div className={style['login-form-extra']}>
        <p>
          Not an admin? <a href="https://www.dev.corecare.in/login">Login</a> to Corecare for
